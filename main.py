...
async def load_user_session() -> Client | None:
    global user
    if not os.path.exists(SESSION_STR_FILE):
        return None
    try:
        s = open(SESSION_STR_FILE).read().strip()
        if not s:
            return None
        u = Client(SESSION_FILE, api_id=config.API_ID,
                   api_hash=config.API_HASH, session_string=s)
        await u.start()
        user = u
        return u
...
async def cmd_logout(_, msg: Message):
    ...
    await msg.reply_text("✅ Logout ho gaya.")
...
# Kisi bhi bot ka SAARA content ek baar mein
# ══════════════════════════════════════════════════════════════════════════════
...
async def cmd_fetchall(_, msg: Message):
    ...
    Us bot ke saath aapki poori chat history check karega.
    Jo bhi videos, PDFs, photos milenge — sab aapko bhej dega.
    """
    if not is_owner(msg):
    ...
    try:
        # Fetch complete chat history with this bot
        total_media = 0
        total_scanned = 0
        saved = 0
        failed = 0
        ...
        async for hist_msg in u.get_chat_history(botname):
            ...
            # Progress update har 5 media pe
            if total_media % 5 == 0:
                try:
        ...
        await status.edit_text(
            f"❌ Error aaya:\n`{str(e)}`\n\n"
            "Possible reason: Aapka account @{botname} se pehle connected nahi hai.\n"
            "Pehle us bot pe /start bhejo, fir dobara try karo."
        )
    finally:
...
# ══════════════════════════════════════════════════════════════════════════════
# /fetch @botname <command>  — Specific command ka response
# ══════════════════════════════════════════════════════════════════════════════
    ...
    try:
        ...
        if not found:
            await status.edit_text(
                ...
                "Dobara try karo ya `/fetchall @{botname}` use karo."
            )
    except Exception as e:
...
async def main():
    ...
    if restored:
        ...
        logger.info(f"Session restored: {me.first_name}")
    else:
        logger.info("No session — use /login")
